...
//
// The trust core for runtime-loaded editor plugins. Open-core knows nothing about what a plugin
// does — it ships ONLY these pure verifiers + the baked-in public key, and the desktop/CLI MUST
// call them before importing any fetched bundle file:
//
...
// key signs leases + the plugin bundle; rotating it means shipping a new app build.

import { createHash, createPublicKey, verify } from "node:crypto";

/** The plugin-signing public key (Ed25519, SPKI PEM). Baked at build time (the release injects it,
 *  like the telemetry key); overridable via env for dev / self-host. Empty ⇒ nothing verifies, so
 *  no plugin ever loads (fail-closed). */
export const PLUGIN_PUBLIC_KEY: string = process.env.INPLAN_PLUGIN_PUBLIC_KEY ?? "";
...
/** Verify + decode an entitlement lease (`base64url(payload).base64url(ed25519sig)` — the format the
 *  plugin server's signLease emits). Returns the claims iff the signature matches the public key AND
 *  `exp` is in the future; otherwise null. Never throws. */
export function verifyLease(token: string, publicKeyPem: string = PLUGIN_PUBLIC_KEY, now: number = Date.now()): LeaseClaims | null {
  try {
...
export function verifyBundleBytes(bytes: Buffer, entry: BundleManifestEntry, publicKeyPem: string = PLUGIN_PUBLIC_KEY): boolean {
  try {

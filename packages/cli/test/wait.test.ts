...
import { appendLog, FsControlChannel, LogEventType } from "@inplan/core/node";
...
let logPath: string;
let lockPath: string;
let channel: FsControlChannel;

beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "ap-wait-"));
  logPath = join(dir, "doc.log.jsonl");
  lockPath = join(dir, "doc.waitlock");
  channel = new FsControlChannel({ logPath, cursorPath: join(dir, "doc.cursor"), waitLockPath: lockPath });
});
...
describe("waitForActions", () => {
  it("resolves after a user action and batches surrounding entries", async () => {
    ...
    const pending = waitForActions({ channel, cursor: 0, debounceMs: 40, pollMs: 5 });
  ...
  it("does not declare editorGone if the editor was never seen alive", async () => {
    ...
    const pending = waitForActions({ channel, cursor: 0, debounceMs: 40, pollMs: 10, watchEditor: true, signal: ac.signal });
  ...
  it("steps down (superseded) when the wait-lock token changes", async () => {
    ...
    const pending = waitForActions({ channel, cursor: 0, debounceMs: 40, pollMs: 5, watchEditor: false, token: "waiter-A" });
  ...
  it("keeps waiting while it still holds the lock", async () => {
    ...
    const ac = new AbortController();
    const pending = waitForActions({ channel, cursor: 0, debounceMs: 40, pollMs: 5, watchEditor: false, token: "waiter-A", signal: ac.signal });
  ...
  it("can be aborted", async () => {
    const ac = new AbortController();
    const pending = waitForActions({ channel, cursor: 0, debounceMs: 40, pollMs: 5, watchEditor: false, signal: ac.signal });
